...
public class Solution {
    ...
    public int minTotalDistance(int[][] grid) {
        ...
        // NOTE : Travel in such a way such that your row and col is in sorted Order
        // Else you will have to sort for taking out median

        // Row wise travel karo and jitne v 1's mil rahe hai unke row idx ko list me bhr k le aao
        List<Integer> rowInSortedOrder = new ArrayList<>();
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++) {
                if(grid[i][j] == 1) {
                    rowInSortedOrder.add(i);
                }
            }
        }

        // Col wise travel karo and jitne v 1's mil rahe hai unke col idx ko list me bhr k le aao
        List<Integer> colInSortedOrder = new ArrayList<>();
        for(int j = 0; j < m; j++) {
            for(int i = 0; i < n; i++) {
                if(grid[i][j] == 1) {
                    colInSortedOrder.add(j);
                }
            }
        }

        // Finding median of all the row idx's and col idx's
        int medianOfRowIdx = rowInSortedOrder.get(rowInSortedOrder.size() / 2);
        int medianOfColIdx = colInSortedOrder.get(colInSortedOrder.size() / 2);

        // Calculating Manhattan distance
        int totalDis = 0;
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++) {
                if(grid[i][j] == 1) { // Jitne v 1's hai sab apna apna dis add kar do in totalDis using Manhattan formula
                    totalDis += Math.abs(medianOfRowIdx - i) + Math.abs(medianOfColIdx - j); // Manhattan
                }
            }
        }
        return totalDis;
