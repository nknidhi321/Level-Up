...
class Solution {
    ...
    public int cherryPickup(int[][] grid) {
        ...
        int ans = cp(0, 0, 0, grid, dp); // Implementing 2 bnde from (0, 0)
        return ans == Integer.MIN_VALUE ? 0 : ans;
    }
    
    public int cp(int r1, int c1, int r2, int[][] grid, Integer[][][] dp) {
        int c2 = r1 + c1 - r2; // r1 + c1 = r2 + c2 => No. of jumps will always be equal
        
        // out of bounds
        if(r1 >= n || r2 >= n || c1 >= n || c2 >= n || grid[r1][c1] == - 1 || grid[r2][c2] == -1) return Integer.MIN_VALUE;
        
        // destination
        if(r1 == n - 1 && c1 == n - 1 && r2 == n - 1 && c2 == n - 1) return dp[r1][c1][r2] = grid[r1][c1];
        ...
        // 2 * 2 ways for 2 people
        int max = Integer.MIN_VALUE;
        max = Math.max(max, cp(r1 + 1, c1, r2 + 1, grid, dp)); // h, h
        max = Math.max(max, cp(r1, c1 + 1, r2, grid, dp)); // v, v
        max = Math.max(max, cp(r1 + 1, c1, r2, grid, dp)); // h, v
        max = Math.max(max, cp(r1, c1 + 1, r2 + 1, grid, dp)); // v, h
        
        // destination pe kvi jaa he nai paaye, (n-1, m-1) cell blocked hai
        if(max == Integer.MIN_VALUE) return dp[r1][c1][r2] = Integer.MIN_VALUE;
        else { // destination se waapas aa kar yaha gire hai
            if(r1 == r2 && c1 == c2) return dp[r1][c1][r2] = max + grid[r1][c1]; // same cell pe hai dono  
            else return dp[r1][c1][r2] = max + grid[r1][c1] + grid[r2][c2]; // diff cell pe hai dono
        }
    }
