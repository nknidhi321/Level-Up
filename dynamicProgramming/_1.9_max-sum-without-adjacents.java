...
class Solution {
    ...
    public int findMaxSum(int arr[], int n) {
        ...
        for(int i = 1; i < n; i++) {
            int currIncSum = excSum + arr[i]; // curr bnde ko inc krna h, so pichla inc nahi le saktey
            int currExcSum = Math.max(incSum, excSum); // curr bnde ko exc krna h, so pichle wale dono inc exc ka max lelo
